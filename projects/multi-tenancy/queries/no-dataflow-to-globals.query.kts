...
// Requirement: Data flows from user requests are not stored in global variables (since they are assumed to be domain-independent) – or they are deleted after the request is answered

fun statement1(tr: TranslationResult): QueryTree<Boolean> {
    return tr.allExtended<HttpRequest> { request ->
...
/**
 * Determines that the reference is always cleared with a [DeAllocate] operation.
 */
