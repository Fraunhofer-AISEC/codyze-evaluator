 ...
 */
package de.fraunhofer.aisec.codyze.passes.concepts.memory.openstack

import de.fraunhofer.aisec.codyze.passes.concepts.config.openstack.OsloConfigPass
import de.fraunhofer.aisec.codyze.passes.concepts.flows.python.*
...
class StevedoreDynamicLoadingPass(ctx: TranslationContext) : ConceptPass(ctx) {
     ...
     *   Ideally when we invoke [OsloConfigPass] and [IniFileConfigurationSourcePass] beforehand,
     *   there should be already DFG edges linking the code with the configuration, so that we can
