...
/**
 * This function checks if the authorization of the [HttpEndpoint] [result] is based on the project
 * or domain of the user. It retrieves all [HttpEndpoint]s with an authorization and checks if the
 * target values (e.g., user ID, project ID) are used in the authorization check.
 */
...
/**
 * This checks, if when a user reads data from or writes data to a database, the user’s domain is
 ...
 */
context(TranslationResult)
fun unauthorizedResponseFromAnotherDomainQuery(
...
/**
 * Checks if an [HttpEndpoint] only throws "Not Authorized" exceptions of a permitted class and none
 * of the disallowed messages appear.
 */
fun HttpEndpoint.onlyThrowsNotAuthorized(policy: UnauthorizedResponsePolicy): QueryTree<Boolean> {
